---
title: Certificaat reacties ophalen | Microsoft Docs
description: ''
keywords: ''
author: billmath
ms.author: billmath
manager: daveba
ms.date: 09/26/2017
ms.topic: reference
ms.prod: microsoft-identity-manager
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 75c92bd31cda28c5eaef7d16832c819e27737525
ms.sourcegitcommit: 7e8c3b85dd3c3965de9cb407daf74521e4cc5515
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 03/10/2020
ms.locfileid: "92758839"
---
# <a name="get-certificate-responses"></a>Certificaat reacties ophalen
Hiermee wordt een set certificaat aanvragen verzonden naar de certificerings instantie (CA) voor verwerking.

>[!IMPORTANT]
>Het aantal certificaat reacties kan groter zijn dan het aantal aanvragen. Sommige Profiel sjablonen kunnen worden geconfigureerd voor het genereren van sleutels/aanvragen op de server of voor het hebben van externe certificaten.

>[!NOTE]
>De Url's in dit artikel zijn relatief ten opzichte van de hostnaam die wordt gekozen tijdens de implementatie van de API, zoals `https://api.contoso.com` .

## <a name="request"></a>Aanvraag

Methode  |Aanvraag-URL  
---------|---------
POST     |/CertificateManagement/api/v1.0/requests/{id}/certificatedata

### <a name="url-parameters"></a>URL-parameters

Parameter | Beschrijving
--------|--------------
id| Vereist. De GUID die overeenkomt met de profiel sjabloon waaruit het beleid moet worden opgehaald.

### <a name="request-headers"></a>Aanvraagheaders
Zie [HTTP-aanvraag-en reactie headers](certificate-management-rest-api-service-details.md#http-request-and-response-headers) in *cm rest API service Details* voor algemene aanvraag headers.

### <a name="request-body"></a>Aanvraagbody
De aanvraag tekst bevat de volgende eigenschappen:

Eigenschap | Beschrijving
---------|-----------
pfxpassword | Het wacht woord dat wordt gebruikt voor het beveiligen van de PFX-resultaten (Personal Information Exchange) die worden geretourneerd door de aanroep.
certificaterequests | De lijst met certificaat aanvragen die door de client worden gegenereerd. De aanvragen moeten worden gegenereerd met behulp van de para meters die zijn opgegeven in het CertificateRequestGenerationOptions-object. Zie [Opties voor gegenereerde certificaat aanvragen ophalen](get-certificate-request-generation-options.md)voor meer informatie.
aantekeningen | Een teken reeks met aanvullende opmerkingen die worden vastgelegd in de gebeurtenis geschiedenis van de bewerking.


## <a name="response"></a>Antwoord
In deze sectie wordt het antwoord beschreven.

### <a name="response-codes"></a>Antwoord codes

Code  |Beschrijving  
---------|---------
200 | OK
403 | Verboden
500 | Interne fout

### <a name="response-headers"></a>Antwoordheaders
Zie [HTTP-aanvraag-en reactie headers](certificate-management-rest-api-service-details.md#http-request-and-response-headers) in *cm rest API service Details* voor algemene antwoord headers.

### <a name="response-body"></a>Hoofdtekst van de reactie
Als de bewerking is voltooid, wordt een lijst met JSON-geserialiseerde [micro soft. clm. Shared. certificates. certificaat](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) -objecten geretourneerd met de volgende eigenschappen:

Eigenschap| Beschrijving
------|-------------
CertificateId | De certificaat-ID.
Fout | Er is een fout opgetreden bij het verwerken van de certificaat aanvraag.
Markering | De vlaggen die het certificaat beschrijven.
IsExternal | Een Booleaanse waarde die aangeeft of het certificaat een extern certificaat is.
isKeyHistory | Een Booleaanse waarde die aangeeft of het certificaat een sleutel geschiedenis certificaat is.
isPfx | Een Booleaanse waarde die aangeeft of de reactie van de certificaat aanvraag in de PFX-indeling is.
isPkcs7 | Een Booleaanse waarde die aangeeft of het antwoord van de certificaat aanvraag in PKCS # 7-indeling is.
isServerGenerated | Een Booleaanse waarde die aangeeft of het certificaat is gegenereerd door een server.
Pfx | De PFX-certificaat-blob.
Pkcs7 | De PKCS # 7-certificaat-blob.
TemplateCommonName | De algemene naam van het certificaat sjabloon voor het certificaat.
UseLocalMachineStore | Een Booleaanse waarde die aangeeft of het certificaat is voor het certificaat archief van de lokale computer.

## <a name="example"></a>Voorbeeld
In deze sectie vindt u een voor beeld van het indienen van een set certificaat aanvragen bij de CA voor verwerking.

### <a name="example-request"></a>Voor beeld: aanvraag

```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}
```

### <a name="example-response"></a>Voor beeld: antwoord

```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```

## <a name="see-also"></a>Zie ook

- [Microsoft.Clm.Provision.ExecuteOperations. REGI ster-methode](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [De klasse micro soft. clm. Shared. certificaten. Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
