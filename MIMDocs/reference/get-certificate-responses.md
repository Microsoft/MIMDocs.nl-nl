---
title: Certificaat antwoorden ophalen | Microsoft Docs
description: 
keywords: 
author: msmbaldwin
ms.author: mbaldwin
manager: mbaldwin
ms.date: 10/17/2016
ms.topic: reference
ms.prod: identity-manager-2015
ms.service: microsoft-identity-manager
ms.technology: security
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 3ac905a25e41a6212efb5f59e2f47e4345382ec3
ms.sourcegitcommit: 02fb1274ae0dc11288f8bd9cd4799af144b8feae
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 07/13/2017
---
# <a name="get-certificate-responses"></a>Antwoorden certificaat ophalen
Verzendt een reeks aan de certificeringsinstantie (CA) voor de verwerking van aanvragen voor certificaten.

**Opmerking**: het aantal certificaat antwoorden mogelijk het aantal aanvragen overschrijden, omdat sommige profielsjablonen mogelijk geconfigureerd voor het genereren van sleutels/aanvragen op de server of externe certificaten hebben.

**Opmerking**: URL's weergegeven in dit onderwerp zijn ten opzichte van de hostnaam van de gekozen tijdens de implementatie van de API; bijvoorbeeld: `https://api.contoso.com`.
##<a name="request"></a>Aanvraag


Methode  |Aanvraag-URL  
---------|---------
POST     |/CertificateManagement/API/V1.0/Requests/{id}/certificatedata

###<a name="url-parameters"></a>URL-Parameters
Parameter | Beschrijving
--------|--------------
ID| Vereist. De GUID die overeenkomt met de profielsjabloon die het beleid moet worden opgehaald uit.

###<a name="request-headers"></a>Aanvraagheaders
Raadpleeg voor algemene aanvraagheaders [HTTP-aanvraag en Reactieheaders](certificate-management-rest-api-service-details.md#http-request-and-response-headers) in *CM REST API-servicedetails*.
###<a name="request-body"></a>Aanvraagtekst
De aanvraagtekst bevat de volgende eigenschappen.

Eigenschap | Beschrijving
---------|-----------
pfxpassword | Het wachtwoord dat wordt gebruikt voor het beveiligen van alle persoonlijke informatie Exchange (PFX)-resultaten die worden geretourneerd van de aanroep.
certificaterequests | De lijst met certificaataanvragen gegenereerd door de client. Deze aanvragen moeten zijn gegenereerd met de parameters die zijn opgegeven in het object CertificateRequestGenerationOptions; Zie voor meer informatie [ophalen aanvragen generaties certificaatopties](get-certificate-request-generation-options.md).
Opmerkingen | Een tekenreeks met aanvullende opmerkingen die worden vastgelegd in het logboek geschiedenis van de bewerking.


##<a name="response"></a>Antwoord
###<a name="response-codes"></a>Reactiecodes
Code  |Beschrijving  
---------|---------
200     | OK
403 | Is niet toegestaan
500 | Interne fout

###<a name="response-headers"></a>Antwoordheaders
Zie voor veelgebruikte antwoordheaders [HTTP-aanvraag en Reactieheaders](certificate-management-rest-api-service-details.md#http-request-and-response-headers) in *CM REST API-servicedetails*.
###<a name="response-body"></a>Antwoordtekst
Indien geslaagd kunt retourneert lijst met JSON-geserialiseerd [Microsoft.Clm.Shared.Certificates.Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) objecten met de volgende eigenschappen:

Eigenschap| Beschrijving
------|-------------
CertificateId | De certificaat-ID.
Fout | Een fout die is opgetreden tijdens het verwerken van de certificaataanvraag.
Vlaggen | De vlaggen die het certificaat te beschrijven.
IsExternal | Een Boolean die aangeeft of het certificaat een extern certificaat is.
isKeyHistory | Een Boolean die aangeeft of het certificaat een certificaat van de geschiedenis van de sleutel is.
isPfx | Een Booleaanse waarde waarmee wordt aangegeven of het antwoord van de aanvraag certificaat in PFX-indeling.
isPkcs7 | Een Boolean die aangeeft of het antwoord van de aanvraag certificaat in de PKCS #7-indeling is.
isServerGenerated | Een Boolean die aangeeft of het certificaat is gegenereerd door een server.
PFX | Het PFX-certificaat-blob.
Pkcs7 | Het PKCS #7-certificaat-blob.
TemplateCommonName | Algemene naam van het certificaatsjabloon voor het certificaat.
UseLocalMachineStore | Een Booleaanse waarde waarmee wordt aangegeven of het certificaat voor het certificaatarchief van de lokale computer.

##<a name="example"></a>Voorbeeld

###<a name="request"></a>Aanvraag
```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}

```
###<a name="response"></a>Antwoord
```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```       
##<a name="see-also"></a>Zie ook

- [Microsoft.Clm.Provision.ExecuteOperations.Enroll methode](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [Microsoft.Clm.Shared.Certificates.Certificate klasse](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
